...
export const releasePart = pgTable("release_part", {
  ...snapshot(), partNumber: text("part_number").notNull(), description: text("description").notNull(), manufacturer: text("manufacturer"), listPrice: money("list_price"), currency: text("currency").notNull(), supersededByPartId: uuid("superseded_by_part_id"), status: text("status", { enum: ["active", "superseded", "discontinued"] }).notNull().default("active"),
}, t => [primaryKey({ columns: [t.releaseId, t.id] }), unique("release_part_source").on(t.releaseId, t.workingId), currencyCheck(t.currency), check("release_part_price", sql`${t.listPrice} >= 0 AND ${t.listPrice} < 'Infinity'::numeric`), check("release_part_status", sql`${t.status} IN ('active','superseded','discontinued')`), check("release_part_no_self", sql`${t.supersededByPartId} <> ${t.id}`), foreignKey({ columns: [t.releaseId, t.supersededByPartId], foreignColumns: [t.releaseId, t.id] })]);
