...
**Interfaces:** `publishModel(ctx, input: PublishInput, idempotencyKey): Promise<PublishResult>` extends the existing input with required `expectedPublicationVersion`. `expectedWorkingVersion` retains source `model.version` semantics; separate `model.publicationVersion` serializes publish/activate/rollback without changing source bindings or invalidating approved geometry. Activate/rollback require the coordination version plus `expectedActiveReleaseId`; scoped queue entries expose both versions. `readPublishedFigure(ctx, figureId): Promise<FigureDetail>` extends existing composite contract with numeric mapping and release identity. API `/api/v1/catalog/figures/:figureId`; publishing/rollback paths follow `docs/backend-specification.md` and must be registered explicitly.
