...
export default defineConfig({
  ...
  // customer-native.ts owns a private headed Chrome profile so real tab-return
  // authority is exercised. Keep successful traces as production-gate evidence.
  outputDir: process.env.CUSTOMER_OUTPUT_DIR ?? "output/customer-browser", use: { baseURL: process.env.CUSTOMER_BASE_URL ?? "http://127.0.0.1:3199", actionTimeout: 15000, navigationTimeout: 15000, screenshot: "only-on-failure", trace: "on" },
