...
import { DraftFigurePageSchema, PublicationQueuePageSchema, PublishResultSchema, type PublishInput } from "@rufdiamond/contracts";
import { MappingApiError } from "./api-client";

export function createAdminApiClient(csrfToken: string) {
  async function send<S extends TSchema>(path: string, schema: S, body?: PublishInput, key?: string): Promise<Static<S>> {
    const response = await fetch(`/api/v1/admin/${path}`, { method: body ? "POST" : "GET", credentials: "same-origin", cache: "no-store", redirect: "error", headers: body ? { "Content-Type": "application/json", "X-CSRF-Token": csrfToken, "Idempotency-Key": key! } : {}, ...(body ? { body: JSON.stringify(body) } : {}) });
    let result: unknown;
