...
export function useDiagramSelection({ figureId, releaseKey, rows }: {
  ...
  const [state, setState] = useState<{ figureId: string; releaseKey: string; selection: DiagramSelection }>(
  ...
  const selectPart: SelectDiagramPart = useCallback((figurePartId) => {
    setState({ figureId, releaseKey, selection: resolveDiagramSelection(figureId, figurePartId, rows) });
  ...
  return { selection, selectPart, clear, selectedPartIds };
}
