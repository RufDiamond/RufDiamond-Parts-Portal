...
test("unchanged authority preserves mounted quote notes, exclusions and machine across real tab return", async ({ page, baseURL }, testInfo) => {
  ...
  await page.getByRole("button", { name: "Add general comments", exact: true }).click();
  await page.getByRole("textbox", { name: "General comments" }).fill("Keep the general note");
  await page.getByRole("checkbox", { name: "Include SYN-PART", exact: true }).uncheck();
  await expect(page.getByRole("cell", { name: "Synthetic machine", exact: true })).toBeVisible();
  const document = await page.evaluate(() => performance.timeOrigin);
  let release!: () => void;
  const pending = new Promise<void>(resolve => { release = resolve; });
  await page.route("**/api/v1/me", async route => { await pending; await route.continue(); });
  const otherTab = await page.context().newPage();
  await otherTab.bringToFront();
  await expect.poll(() => page.evaluate(() => window.document.visibilityState)).toBe("hidden");
  await page.bringToFront();
  await expect.poll(() => page.evaluate(() => window.document.visibilityState)).toBe("visible");
  await expect(page.getByRole("status", { name: "" })).toContainText("Checking catalogue access");
  await expect(page.getByRole("textbox", { name: "General comments" })).toHaveCount(0);
  release();
  await expect(page.getByRole("textbox", { name: "General comments" })).toHaveValue("Keep the general note");
  await expect(page.getByRole("textbox", { name: "Comment on SYN-PART" })).toHaveValue("Keep the line note");
  await expect(page.getByRole("checkbox", { name: "Include SYN-PART", exact: true })).not.toBeChecked();
  await expect(page.getByRole("cell", { name: "Synthetic machine", exact: true })).toBeVisible();
  expect(await page.evaluate(() => performance.timeOrigin)).toBe(document);
  await page.screenshot({ path: testInfo.outputPath("unchanged-authority-quote.png"), fullPage: true });
